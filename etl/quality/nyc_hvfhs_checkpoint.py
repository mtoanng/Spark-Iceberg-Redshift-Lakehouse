...
def evaluate_fixture_checkpoint(
    bronze: BronzeBatch, silver: SilverBatch
    ...
    missing_reason = [
        row for row in silver.quarantine_rows if not row.get("reason_code")
    ]
    if missing_reason:
        raise QualityCheckpointError("Quarantine rows must include a reason_code.")

    row_ids = [str(row.get("row_id", "")) for row in silver.silver_rows]
    if not all(row_ids):
        raise QualityCheckpointError("Silver rows must include row_id.")
    if len(set(row_ids)) != len(row_ids):
        raise QualityCheckpointError("Silver row_id values must be unique.")
    ...
    return QualityCheckpointResult(
        ...
        distinct_row_count=len(set(row_ids)),
    )
