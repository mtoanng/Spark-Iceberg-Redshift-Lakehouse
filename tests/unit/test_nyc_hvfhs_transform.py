...
def test_bronze_preserves_source_columns_and_adds_only_metadata() -> None:
    ...
    assert bronze_record["_source_uri"] == SOURCE.source_uri
    ...
    assert len(bronze_record["row_id"]) == 64
    assert len(bronze_record["business_trip_key"]) == 64
    assert bronze_record["identity_policy_version"] == "nyc-hvfhv-row-v1-2024"
...
def test_reconciliation_explains_every_bronze_row() -> None:
    ...
    assert (counts.bronze_count, counts.silver_count, counts.quarantine_count) == (
        5,
        1,
        4,
    )
