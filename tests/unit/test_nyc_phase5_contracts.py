...
from etl.quality.nyc_hvfhs_checkpoint import QualityCheckpointError, evaluate_fixture_checkpoint
from etl.sources.nyc_hvfhs import SourceContractError, SourceFile, SourceManifestEntry, manifest_decision, stable_run_id
from etl.transforms.nyc_hvfhs import BronzeBatch, SilverBatch, bronze_records, load_zone_ids, transform_silver


FIXTURE_DIR = Path(__file__).parents[1] / "fixtures" / "nyc_hvfhs"
SOURCE = SourceFile(2024, 1, "fixture:///fhvhv_tripdata_2024-01.parquet", "fixture-checksum", 2689)


def _fixture_batch() -> tuple[BronzeBatch, SilverBatch]:
    records = json.loads((FIXTURE_DIR / "fhvhv_tripdata_2024-01.fixture.json").read_text(encoding="utf-8"))["records"]
...
def test_quality_checkpoint_accepts_reconciled_fixture() -> None:
    ...
    assert (result.bronze_count, result.silver_count, result.quarantine_count, result.distinct_trip_count) == (5, 1, 4, 1)
...
def test_quality_checkpoint_rejects_duplicate_canonical_trip() -> None:
    ...
    duplicated_silver = replace(silver, silver_rows=(silver.silver_rows[0], silver.silver_rows[0]), quarantine_rows=())
...
def test_quality_checkpoint_rejects_quarantine_without_reason() -> None:
    ...
    broken_silver = replace(silver, quarantine_rows=(missing_reason, *silver.quarantine_rows[1:]))

    with pytest.raises(QualityCheckpointError, match="reason_code"):
        evaluate_fixture_checkpoint(bronze, broken_silver)


def test_three_month_backfill_is_sequential_and_bounded() -> None:
    ...
    with pytest.raises(SourceContractError, match="October"):
        sequential_backfill_requests(2024, 11)
...
def test_identical_rerun_skips_then_force_requests_same_source_retry() -> None:
    ...
    assert manifest_decision(processed, SOURCE, force=False).value == "skip_identical_processed_source"
    assert manifest_decision(processed, SOURCE, force=True).value == "process_forced_retry"
