...
# Note: DUCKDB_ROLE_ARN is NOT needed - Docker mounts ~/.aws, DuckDB uses IAM user credentials directly
```
...
USE_GLUE_CATALOG=true
# Note: DUCKDB_ROLE_ARN removed - Docker mounts ~/.aws, uses IAM user credentials
AWS_REGION=<terraform output -raw aws_region>
AWS_ACCOUNT_ID=<terraform output -raw aws_account_id>
```

### **9.3 Run ML Training & Generate Recommendations**

```powershell
# Run Spark ML Glue Job (trains model + generates recommendations)
aws glue start-job-run --job-name instacart-lakehouse-ml-recommendations --arguments="--MONGODB_URI=%MONGODB_URI%,--MONGODB_DATABASE=instacart_warehouse,--TOP_N=10"
```

**Expected output:**
```json
{
    "JobRunId": "jr_ml_abc123..."
}
```

**📝 WRITE DOWN JobRunId:**
```
ML_JOB_RUN_ID: ________________________________
```

**Monitor job status:**
```powershell
aws glue get-job-run --job-name instacart-lakehouse-ml-recommendations --run-id jr_ml_abc123... --query "JobRun.JobRunState" --output text
```

**Expected duration:** 15-20 minutes

**Expected output from CloudWatch logs:**
```
🚀 Starting Spark ML Recommendation Job
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
🎯 Training Spark ML LogisticRegression model...
   Parameters:
     - maxIter: 30
     - regParam: 0.05
...
🔮 Predicting reorder probabilities for all user-product pairs...
✅ Predictions complete

📋 Generating top-10 recommendations per user...

💾 Writing recommendations to MongoDB Atlas...
...
✅ Spark ML recommendation job completed successfully!
```

### **9.4 Verify Recommendations in MongoDB**
...
**✅ Checkpoint 9:** Spark ML Glue Job completed, 206K users with recommendations in MongoDB Atlas
...
# 4. ML Model
Write-Host '4️⃣  ML Recommendation Job...' -ForegroundColor Yellow
# Check if ML Glue job exists
$mlJob = aws glue get-job --job-name instacart-lakehouse-ml-recommendations --query 'Job.Name' --output text 2>$null
if ($mlJob -eq 'instacart-lakehouse-ml-recommendations') {
    Write-Host '   ✅ ML Glue Job exists' -ForegroundColor Green
} else {
    Write-Host '   ❌ ML Glue Job missing' -ForegroundColor Red
}
...
- **ML:** Spark ML LogisticRegression (AWS Glue Job)
...
echo %AWS_REGION%
echo %AWS_ACCOUNT_ID%
# Note: DUCKDB_ROLE_ARN removed - Docker mounts ~/.aws
...
**Symptom:**
```
pymongo.errors.ServerSelectionTimeoutError: Cannot connect to MongoDB Atlas
```

**Solution:**
```powershell
# Check MONGODB_URI in .env is correct (MongoDB Atlas connection string)
...
• Built production-ready data lakehouse on AWS processing 33M+ e-commerce records 
  ...
  <4-hour end-to-end pipeline with Spark ML model (AUC ~0.83) generating 
...
- Spark ML pipeline (12 features, LogisticRegression)
...
✅ Spark ML model (AUC ~0.83) for product recommendations
...
Tech stack: AWS Glue | Iceberg | dbt | Spark ML | DuckDB | FastAPI | MongoDB | Terraform
...
- [ ] ML Glue Job run successfully (SUCCEEDED)
- [ ] Recommendations generated and saved to MongoDB Atlas
...
✅ ML-powered recommendation engine (Spark ML)  
...
✅ Python (PySpark, FastAPI, Spark ML)  
...
# 10. Run ML & Generate Recommendations (AWS Glue Job)
aws glue start-job-run --job-name instacart-lakehouse-ml-recommendations --arguments="--MONGODB_URI=%MONGODB_URI%,--MONGODB_DATABASE=instacart_warehouse"
# Wait for completion (~20 min)
